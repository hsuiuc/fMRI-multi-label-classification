...
prediction = np.load(sys.argv[2]).astype('float32')
...
if sys.argv[1].lower() == "subset":
    ...
    #            ",".join(tag_name), delimiter=",", fmt='%.10f', comments='')
    with open(sys.argv[3], 'w') as f:
        print("id,%s" % ",".join(tag_name), file=f)
        for i in range(1971):
            strings = ['%.10f' % num for num in prediction[i]]
            print("%d,%s" % (i, ",".join(strings)), file=f)
